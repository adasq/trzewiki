...
class User
{
	//user_id		username	mail	password	reg_date 		is_active
	...
	public $reg_date;
	public $is_active;
	public $last_mess_activity;

	const USER_DEFAULT_AVATAR='user_default_100x100.png';
	//----------------------------------------------------------------------------------------------------------------------

	public function __construct()
	{
		$this->avatar=User::USER_DEFAULT_AVATAR;
		$this->reg_date=date("Y-m-d H:i:s");
		$this->last_mess_activity=null;
	}//__construct
	...
	public function getFriendsList(){
		...
		$sql=
		"SELECT u.* FROM friends as f, users as u WHERE f.user1=".$this->user_id." and f.mode=1 and u.user_id = f.user2".
		" union ".
	...
	function getCatalogTree(){
		...
		for ($i=0;$i<sizeof($catalogs);$i++){
			if($catalogs[$i]->height == $currentHeight){
				...
				$arr=array();
				$arr[]=$catalogs[$i];
				if($i+1==sizeof($catalogs))$main[]=$arr;
			}
	...
	public function __toString(){
		...
		<tr><td>password:</td><td>$this->password</td></tr>
		<tr><td>reg_date:</td><td>$this->reg_date</td></tr>
		<tr><td>is_active:</td><td>$this->is_active</td></tr>
		<tr><td>avatar:</td><td>$this->avatar</td></tr>
		<tr><td>last_mess_activity:</td><td>$this->last_mess_activity</td></tr>
		</table>";
	}//__toString
	//----------------------------------------------------------------------------------------------------------------------
}//class
